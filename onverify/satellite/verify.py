...
class Parser(object):
    ...
    def define_parser(self):
        ...
        self.parser.add_argument("ncglob", help="Glob pattern for netcdf files to process")
        self.parser.add_argument("-o", "--outdir", default='./output', help="path for output files")
        self.parser.add_argument("-nrt", "--nrt", type=str, default='', help="Use raw or ifremer near real time data. By defalt cersat is used")
        self.parser.add_argument("-obs", "--obsregex", default=None, help="Observations locations")
        self.parser.add_argument("-b", "--boxsize", type=float, default=2, help="Lat/lon grid size for stats calculations")
        self.parser.add_argument("-vmin", "--vmin", type=float, default=0, help="Plot min")
        self.parser.add_argument("-vmax", "--vmax", type=float, default=6, help="Plot max")
        self.parser.add_argument("-x1", "--lonmin", type=float, default=None, help="lonmin for delimitting verification area")
        self.parser.add_argument("-x2", "--lonmax", type=float, default=None, help="lonmax for delimitting verification area")
        self.parser.add_argument("-y1", "--latmin", type=float, default=None, help="latmin for delimitting verification area")
        self.parser.add_argument("-y2", "--latmax", type=float, default=None, help="latmin for delimitting verification area")
        self.parser.add_argument("-n", "--ncores", type=int, default=1, help="Number of cores to use in colocation calculations")
        self.parser.add_argument("-m", "--model_vars", default=[], nargs="*", help="model variables to interpolate collocs dataframe. First should be hs or wndsp")
        self.parser.add_argument("-k", "--keepna", action="store_true", help="choose it to keep nans after collocating obs and model")

    def parsecla(self):
        self.args = self.parser.parse_args()

    def action_args(self):
        assert glob(self.args.ncglob), 'No netcdf file located to process in %s' % (self.args.ncglob)
        ...
        if self.args.nrt == 'raw':
            ...
            cla = Verify
        calcColocs(self.args.ncglob, obsregex=self.args.obsregex, outdir=colocsdir, cla=cla,
                model_vars=self.args.model_vars,lonmin=self.args.lonmin, lonmax=self.args.lonmax,
                latmin=self.args.latmin, latmax=self.args.latmax, dropna=not self.args.keepna,
                pool=self.args.ncores)
        createPlots(hdfglob=colocsdir+'/*pkl', plotdir=plotdir,
                boxsize=self.args.boxsize, vmin=self.args.vmin, vmax=self.args.vmax)
...
class Verify(VerifyBase):
    ...
    def loadModel(self, ncglob):
        ...
        extra_vars = {'uwnd', 'vwnd', 'ugrd10m', 'vgrd10m', 'ucur', 'vcur', 'u10', 'v10'} # vector variables can be built later
        ...
        self.model = self.model.sel(**{self.latname: slice(self.latmin, self.latmax),
        ...
        self.latres = np.abs(self.model[self.latname].values[0] -
                             self.model[self.latname].values[1])
        self.lonres = np.abs(self.model[self.lonname].values[0] -
                             self.model[self.lonname].values[1])
    ...
    def loadObs(self, interval=timedelta(hours=24), dropvars=None):
        ...
        self.altnames = {1: 'ERS1', 2: 'ERS2', 3: 'ENVISAT', 4: 'TOPEX',
                         5: 'POSEIDON', 6: 'JASON1', 7: 'GFO', 8: 'JASON2',
                         9: 'CRYOSAT'}
        obslist = []
        dttmp = self.t0
        ...
        if not dropvars:
            dropvars = set(['swhcor', 'swhstd', 'sigma0_cal', 'pass_number', 'mes', 'sigma0',
                            'sigma0std', 'sigma0second', 'cycle', 'swh', 'wind_speed',
                            'wind_speed_cor', 'absolute_orbit', 'sigma0secondstd',
                            'sigma0second']).difference(obsvars)
        while dttmp <= self.t1:
        ...
        if not obslist:
            raise IOError('No obs between %s-%s (%s)' % (self.t0, self.t1, self.obsregex))
        ...
        self.obs = self.obs.loc[(self.obs.lat >= self.modlatmin) &
        ...
        self.obs.rename(columns={'swhcor':'hs', 'wind_speed_cor': 'wndsp'}, inplace=True)
    ...
    def interpModel(self, passes=1):
        ...
        for modelvar in self.model_vars:
            ...
            if modelvar == 'wndsp' and {'ugrd10m', 'vgrd10m'}.issubset(self.model.var()):
                self.model[modelvar] = np.sqrt(self.model.ugrd10m**2 + self.model.vgrd10m**2)
            elif modelvar == 'wndsp' and {'uwnd', 'vwnd'}.issubset(self.model.var()):
                self.model[modelvar] = np.sqrt(self.model.uwnd**2 + self.model.vwnd**2)
            elif modelvar == 'wndsp' and {'u10', 'v10'}.issubset(self.model.var()):
                self.model[modelvar] = np.sqrt(self.model.u10**2 + self.model.v10**2)
            elif modelvar == 'cur' and {'ucur', 'vcur'}.issubset(self.model.var()):
                self.model[modelvar] = np.sqrt(self.model.ucur**2 + self.model.vcur**2)
            elif modelvar not in self.model.data_vars:
                self.logger.warn("%s not in model dataset, skipping interpolation" % (modelvar))
                continue
    ...
    def _calc_interp_indeces(self):
        ...
        if self.model[self.latname][0] < self.model[self.latname][1]:
            latindex = np.round((1/float(self.latres)) *
                                (self.obs.lat.values - self.modlatmin)).astype(int)
        else:
            latindex = np.round((1/float(self.latres)) *
                                (self.modlatmax - self.obs.lat.values)).astype(int)

        if self.model[self.lonname][0] < self.model[self.lonname][1]:
            lonindex = np.round((1/float(self.lonres)) *
                                (self.obs.lon.values - self.modlonmin)).astype(int)
        else:
            lonindex = np.round((1/float(self.lonres)) *
                                (self.modlonmax - self.obs.lon.values)).astype(int)
    ...
    def _timedelta_to_index(self, timedelta):
        ...
        """
        return ((timedelta.days) * 24.0 + (timedelta.seconds/3600.0)) / (self.tstep.seconds/3600.0)
    ...
    def calcGriddedStats(self, boxsize, latedges=None, lonedges=None):
        ...
        if latedges is None:
            minlat, maxlat = lat.min()+(abs(lat.min()) % boxsize), lat.max()-(abs(lat.min()) % boxsize)
            #minlat, maxlat = lat.min(), lat.max()
            lats = np.arange(minlat+boxsize/2., maxlat + boxsize/2., boxsize)
            nlats = lats.size
            latedges = np.arange(minlat, maxlat+boxsize, boxsize)

        if lonedges is None:
            minlon, maxlon = lon.min()+(abs(lon.min()) % boxsize), lon.max()-(abs(lon.min()) % boxsize)
            lons = np.arange(minlon+boxsize/2., maxlon + boxsize/2., boxsize)
            nlons = lons.size
            lonedges = np.arange(minlon, maxlon+boxsize, boxsize)

        dt = np.dtype([('bias', np.float32),
                       ('nbias', np.float32),
                       ('bias2', np.float32),
                       ('rmsd', np.float32),
                       ('nrmsd', np.float32),
        ...
        grid_bias_sum, xedges, yedges = np.histogram2d(lat, lon, weights=diff, bins=(latedges, lonedges))
        grid_bias2_sum, xedges, yedges = np.histogram2d(lat, lon, weights=diff2, bins=(latedges, lonedges))
        grid_bias_sum = ma.masked_equal(grid_bias_sum, 0)
        obs_sum, xedges, yedges = np.histogram2d(lat, lon, weights=obs, bins=(latedges, lonedges))
        obs_sum = ma.masked_equal(obs_sum, 0)
        mod_sum, xedges, yedges = np.histogram2d(lat, lon, weights=model, bins=(latedges, lonedges))
        mod_sum = ma.masked_equal(mod_sum, 0)
        grid_msd_sum, xedges, yedges = np.histogram2d(lat, lon, weights=msdall, bins=(latedges, lonedges))
        ...
        stats_grid = np.rec.fromarrays((grid_bias, nbias, grid_bias2, grid_rmsd, nrmsd,
                                        grid_si, obs_mean, mod_mean, grid_N), dtype=dt)
    ...
    def saveGriddedStats(self, outnc):
        ...
        lat = self.stats_grid_lats if len(self.stats_grid_lats) == nlat else self.stats_grid_lats[:-1]
        lon = self.stats_grid_lons if len(self.stats_grid_lons) == nlon else self.stats_grid_lons[:-1]
        ds = xr.Dataset(coords={'lat': lat, 'lon': lon})
        for stat in ('bias', 'nbias', 'bias2', 'rmsd', 'nrmsd', 'si', 'obsmean', 'modmean', 'N'):
            ds[stat] = (('lat', 'lon'), self.stats_grid[stat])
        ds.to_netcdf(outnc)

    def plotGriddedStats(self, statistic, minobs=100, ptype='pcolormesh', title=False, smoothfac=False, scalefactor=1, **kwargs):
        """
        ...
        if statistic == 'si':
            ...
            cblabel = statistic
        if statistic != 'N':
            data = ma.masked_where(self.stats_grid['N'] <= minobs, self.stats_grid[statistic])
        else:
            data = ma.masked_where(self.stats_grid['N'] <= 0, self.stats_grid[statistic])
        if smoothfac:
            import smooth
    ...
    def calcStatsFreq(self, freq='M'):
        return self.df.groupby([pd.TimeGrouper(freq=freq), 'satellite']).apply(statsDict)
...
class VerifyNRT(Verify):
    ...
    def __init__(self,
        ...
        super(VerifyNRT, self).__init__(logger=logging,
                                        obsregex=obsregex,
                                        test=test,
                                        modvar=modvar,
                                        latmin=latmin,
    ...
    def loadObs(self, interval=timedelta(hours=24), dropvars=None, ):
        ...
        self.altnames = {1: 'ERS1', 2: 'ERS2', 3: 'ENVISAT', 4: 'TOPEX',
                         5: 'POSEIDON', 6: 'JASON1', 7: 'GFO', 8: 'JASON2',
                         9: 'CRYOSAT2'}
        self.altfnames = {1: 'ERS1', 2: 'ERS2', 3: 'ENV', 4: 'TOPEX',
                          5: 'POSEIDON', 6: 'JAS1', 7: 'GFO', 8: 'JAS2',
                          9: 'CRYO'}
        #'swhcor', #'lat', #'lon', #'time' #'satellite',
        ...
        for key, alt in self.altfnames.items():
            ...
            if nfiles == 0:
                ...
                for ii in np.arange(0, fac):
                    self.logger.info("  Opening files %s to %s of %s" % (ii*split, (ii+1)*split, len(obslist)))
                    obsfiles1 = xr.open_mfdataset(obslist[ii*split:(ii+1)*split], drop_variables=dropvars)
                    if first:
...
class VerifyNRTraw(Verify):
    ...
    def __init__(self,
        ...
        super(VerifyNRTraw, self).__init__(logger=logging,
                                           obsregex=obsregex,
                                           test=test,
                                           modvar=modvar,
                                           latmin=latmin,
    ...
    def loadObs(self,):
        ...
        # Loading satellite data
        self.obs = load_nrt(tlims=[self.t0, self.t1],
...
def plotMap(data, lats, lons, ptype='pcolormesh', proj='Robinson', clon=None,
            clat=None, **kwargs):
    if clon == None:
    ...
    if ptype == 'pcolormesh':
        ...
        mpl = ax.contour(lons, lats, data, transform=ccrs.PlateCarree(), zorder=2, **kwargs)
        mpl = ax.contourf(lons, lats, data, transform=ccrs.PlateCarree(), zorder=1, **kwargs)
    else:
...
def createPlots(ncglob=None, hdfglob=None, plotdir='./plots', boxsize=2, vmin=0,
                vmax=8, proj='Robinson', clat=None, clon=None, scalefactor=1, **kwargs):
...
def calcColocs(ncglob, cla=Verify, outdir='./out', overwrite=False,
               dropna=True, pool=1, plot_kwargs=None, **kwargs):
    ...
    if pool == 1:
        for fname in flist:
            calcColocsFile(fname, cla=cla, outdir=outdir, overwrite=overwrite,
                           dropna=dropna, plot_kwargs=plot_kwargs, **kwargs)
    else:
        p = Pool(pool)
        func = partial(calcColocsFile, cla=cla, outdir=outdir, overwrite=overwrite,
                           dropna=dropna, plot_kwargs=plot_kwargs, **kwargs)
        p.map(func, flist)
