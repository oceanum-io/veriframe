...
class AxisVerify(object):
    ...
    def _get_path_coords(self, path):
        ...
        if (vertices[:,0].min() == -0.5 and vertices[:,0].max() == 0.5 and
            vertices[:,1].min() == -0.5 and vertices[:,0].max() == 0.5):
            return offsets
    ...
    def set_xylimit(self, ax, equal=False, offset=0.02, xlim=None, ylim=None):
        ...
        """
        methods = [method for method in dir(self) if method.startswith('_coordinates_from')]
        xdata = []
        ydata = []
...
class VerifyFrame(pd.DataFrame, AxisVerify):
    ...
    _metadata = ['ref_col', 'verify_col', 'var', 'circular', 'lat', 'lon',
                 'ref_label', 'verify_label']
    ...
    def units(self):
        ...
        try:
            ...
            print("Variable {} not implemented in {}. Available variables: "
                  "{}".format(self.var, os.path.abspath(os.path.join(HERE, 'vardef.yml')),
    ...
    def coeffs(self):
        ...
        coef, __ = linear_regression(self[self.ref_col],
    ...
    def _set_axis_label(self, ax):
        ...
        xlabel = '${}_{{{}}}$ ({})'.format(self.var.title(),
                                   self.ref_label.replace('_','-'),
                                   self.units)
        ylabel = '${}_{{{}}}$ ({})'.format(self.var.title(),
                                   self.verify_label.replace('_','-'),
                                   self.units)
    ...
    def _density_matrix(self, binsize):
        ...
        density, __, __ = np.histogram2d(self[self.verify_col],
                                         self[self.ref_col],
                                         bins=(x,x))
        density /= density.max()
    ...
    def _fill_times(self, times):
        ...
        for t in times:
            row = pd.DataFrame(data=OrderedDict((c,np.nan) for c in df.columns),
                               index=[t])
            ...
            if col == 'n':
                ...
                try:
                    ...
                    try:
                        ...
                        raise NotImplementedError('Stat {} has not been defined'.format(col))
    ...
    def si(self, **kwargs):
        ...
        """
        return np.sqrt(((self._err() - self.bias())**2).mean()) / self[self.ref_col].mean()
    ...
    def ks(self, **kargs):
        ...
        sortedcprobs = np.concatenate((cdfref[1], cdfmod[1]))[isort] # sorted by values, the probabilities themselves can be unsorted
    ...
    def plot_scatter(self, showeq=True, colorbar=False,
        ...
        if 'c' in kwargs:
            ...
            df = self.copy(deep=True)
        df.plot(x=self.ref_col,
                y=self.verify_col,
                kind='scatter',
                **kwargs)
        mappable = [chi for chi in kwargs['ax'].get_children()
                    if isinstance(chi, PathCollection)][-1]
    ...
    def plot_qq(self, increment=0.01, showeq=True, xlim=None, ylim=None,
                label=None, **kwargs):
    ...
    def plot_scatter_qq(self, increment=0.01, showeq=True, xlim=None,
                        ylim=None, scatter_kw={}, qq_kw={}, **kwargs):
        ...
        self.plot_qq(increment=increment, showeq=showeq,
                     xlim=xlim, ylim=ylim, **qq_kw)
        return kwargs['ax']

    def plot_scatter_polar(self, theta, radius, colorbar=False, cbar_pad=0.1,
                           oceanographic=True, show_label=False, **kwargs):
        """Polar scatter plot.
        ...
        mappable = [chi for chi in ax.get_children() if
                    isinstance(chi, PathCollection)][-1]
    ...
    def plot_density_hexbin(self, showeq=True, colorbar=True,
                            xlim=None, ylim=None, **kwargs):
    ...
    def plot_density_contour(self, binsize=None, showeq=True, colorbar=True,
                             xlim=None, ylim=None, **kwargs):
    ...
    def plot_density_scatter(self, showeq=True, colorbar=True,
                             xlim=None, ylim=None, **kwargs):
        ...
        if np.any(~np.isnan(zs)):
            ...
            colorbar = False
            logging.warning("No value returned from kde, density scatter cannot "
    ...
    def plot_pdf(self, show_mod=True, show_obs=True, show_hist=False,
                 loc=1, xlim=None, ylim=None, **kwargs):
        ...
        if show_obs:
            ax = self[self.ref_col].plot(kind='density',
                                         label=self.ref_label,
                                         ax=ax,
                                         **kwargs_obs)
            if show_hist:
                ax.hist(self[self.ref_col],
                        normed=True,
                        facecolor=kwargs_obs['color'],
                        label=None,
                        **kwargs_hist)
        if show_mod:
            ax = self[self.verify_col].plot(kind='density',
                                         label=self.verify_label,
                                         ax=ax,
                                         **kwargs_mod)
            if show_hist:
                ax.hist(self[self.verify_col],
                        normed=True,
        ...
        ax.set_xlabel('${}$ ({})'.format(self.var.title(), self.units),
    ...
    def plot_cdf(self, show_mod=True, show_obs=True, loc=2,
                 xlim=None, ylim=None, **kwargs):
    ...
    def plot_timeseries(self, show_mod=True, show_obs=True,
                        fill_under_obs=False, **kwargs):
        ...
        if show_obs:
            if fill_under_obs:
                kwargs_lgd = {x: kwargs_fill[x] for x in kwargs_fill if x not in ['edgecolor']}
                ax.plot(dtimes, self[self.ref_col], label=self.ref_label,
                        **kwargs_lgd)
                ax.fill_between(x=dtimes, y1=self[self.ref_col], y2=0,
                                **kwargs_fill)
            else:
                ax.plot(dtimes, self[self.ref_col], label=self.ref_label,
                        **kwargs_obs)
        if show_mod:
            ax.plot(dtimes, self[self.verify_col], label=self.verify_label,
                    **kwargs_mod)
        ax.grid(is_grid)
        ax.set_ylabel('${}$ ({})'.format(self.var.title(), self.units),
                      fontsize=FONTSIZE_LABEL)
        lgd = self.legend(ax=ax)
        return ax

    def plot_subtimeseries(self, nsubs=None, ndays=None, ymin=None, ymax=None,
                           show_mod=True, show_obs=True, fill_under_obs=False,
                           **kwargs):
        """
        ...
        kwargs_self = {key: getattr(self, key) for key in ['ref_col', 'verify_col', 'ref_label', 'verify_label', 'var']}
        ...
        for isub in range(nsubs):
            ...
            axes.append(vf.plot_timeseries(ax=ax,
                                           show_mod=show_mod,
                                           show_obs=show_obs,
    ...
    def plot_map(self, buff=2, subplot=111, fig=None,
                 etopo=False, levels=[50,300,1000,2000,5000],
                 resolution='110m', land=False, ocean=False, coastline=False,
                 layer='BlueMarble_ShadedRelief_Bathymetry', **kwargs):
        ...
        ax = plot_map(self.lat, self.lon, buff=buff, subplot=subplot, fig=fig,
                 resolution=resolution, etopo=etopo, levels=levels, land=land,
                 ocean=ocean, coastline=coastline, layer=layer, **kwargs)
        return ax
    ...
    def add_regression(self, ax, loc=2, show_eqn=True, show_r2=True, show_n=True, **kwargs):
    ...
    def add_stats(self, ax, decimals=3, loc=2, **kwargs):
        ...
        for stat in self.stats:
            if stat.startswith('n') and len(stat)>1 and getattr(self, '{}'.format(stat[1:]), None):
                val = getattr(self, '{}'.format(stat[1:]))(normalised=True)
            else:
                val = getattr(self, '{}'.format(stat))()
            text += "${} = {}$".format(stat.upper(),
                                         str(np.around(val, decimals=decimals)))
    ...
    def stats_table(self, freq=None, outfile=None, **kwargs):
        ...
        if freq is not None:
            ...
            for group, df in grouped:
                vf = VerifyFrame(df, ref_col=self.ref_col,
                                     verify_col=self.verify_col,
                                     var=self.var)
                ret = pd.concat(
                    (ret, vf._stats_frame(group.strftime(self.time_fmt))),
                    axis=1
                    )
    ...
    @classmethod
    def from_file(cls, filename, kind, ref_col, verify_col, var=None, circular=False,
                 lat=None, lon=None, ref_label=None, verify_label=None, **kwargs):
        ...
        """
        verify_kw = {}
        for key in ['ref_col','verify_col','var','circular',
                    'lat','lon','ref_label','verify_label']:
            val = locals()[key]
            if val is not None:
...
def plot_map(lat, lon, buff=2, bnd=[], subplot=111, fig=None,
             etopo=False, levels=[50,300,1000,2000,5000],
             resolution='110m', land=False, ocean=False, coastline=False,
             layer='BlueMarble_ShadedRelief_Bathymetry', cmap='jet', **kwargs):
    """Plot map of obs location.
    ...
    projection = ccrs.Mercator(central_longitude=float(np.mean(lon)),
    ...
    if isinstance(layer, str):
        ...
        ax.pcolor(layer.lon, layer.lat, np.ma.masked_invalid(layer.values),
                  vmin=None, vmax=None, transform=ccrs.PlateCarree(), cmap=cmap)
    # Add depth contour layer
    if etopo:
        dset = xr.open_dataset(ETOPO).sel(lon=slice(lonmin,lonmax),
                                          lat=slice(latmin,latmax)).load()
        cobj = ax.contour(dset.lon, dset.lat, -dset.z, levels=levels,
                          colors='0.75', transform=ccrs.PlateCarree(), zorder=4)
...
class VerifyFrameMulti(VerifyFrame):
    ...
    # These properties will propagate when subclassing
    _metadata = ['ref_col', 'ref_label', 'verify_cols', 'verify_labels',
                 'plot_colors']
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        # Required arguments
        kwargs.setdefault('ref_col', 'ref_col has not been assigned')                  # Required in both classes
        verify_cols = kwargs.pop('verify_cols', ['verify_cols has not been assigned']) # Required in VerifyFrameMulti
        kwargs.update({'verify_col': verify_cols[0]})                                  # Required in VerifyFrame
        # Optional arguments
        verify_labels = kwargs.pop('verify_labels', verify_cols)                       # Optional in VerifyFrameMulti
        ...
        assert len(self.verify_cols) == len(self.verify_labels), (
    ...
    def _multi_plot(self, plot_function,**kwargs):
        ...
        if not ax:
            ...
            ax = plt.axes()
        ii = 0
        for self.verify_col, self.verify_label in zip(self.verify_cols, self.verify_labels):
            ax = method(ax=ax, color=self.plot_colors[ii],
                        label=self.verify_label, **kwargs)
            ii += 1
        self.legend(loc=loc)
        return ax
    ...
    def plot_set(self, map_buff=20):
        ...
        df2taylor(self, fig=plt.gcf(), obslabel=self.ref_col,
                  mod_cols=self.verify_cols, verify_labels=self.verify_labels,
                  rect='%i%i%i' % (nr,nc,6), colors=self.plot_colors)
    ...
    def plot_set_scatter_density(self, map_buff=20):
        ...
        df2taylor(self, fig=plt.gcf(), obslabel=self.ref_col, rect='%i%i%i' % (nr,nc,6), colors=self.plot_colors)
