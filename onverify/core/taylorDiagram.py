...
def df2taylor(df, obslabel='obs', mod_cols=[], mod_labels=None, fig=None,
    label="Reference", colors=None, plotdir=None, dia=None, legend=True, **kwargs):
    ...
    colors = colors or plt.matplotlib.cm.jet(np.linspace(0,1,(df[mod_cols].isnull().all()==False).sum()))
    ...
    for col in df:
        ...
        dia.add_sample(stddev, R, marker='o', ls='', c=colors[ii],
                           label="%s" % (mapping[col]))
        ii+=1
